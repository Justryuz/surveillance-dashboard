...
export default function SubNav() {
  ...
  return (
    <div className="w-full flex justify-center mb-2">
      <nav className="inline-flex bg-slate-100 dark:bg-slate-950 p-1 rounded-full shadow-inner border border-slate-200 dark:border-slate-800 transition-colors duration-300">
        {menuItems.map((item) => {
          ...
          return (
            ...
            >
              <Icon className={`w-3.5 h-3.5 ${isActive ? 'text-blue-600 dark:text-white' : 'text-slate-400 dark:text-slate-500'}`} />
              {item.name}
            </Link>
          );
        })}
